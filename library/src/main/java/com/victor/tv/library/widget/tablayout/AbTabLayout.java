package com.victor.tv.library.widget.tablayout;
 ...
 */
public abstract class AbTabLayout extends HorizontalScrollView {
    ...
    /**
     * Gravity used to fill the {@link AbTabLayout} as much as possible. This option only takes effect
    ...
    public AbTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.AbTabLayout, defStyleAttr, 0);

        Drawable mStateListDrawable = a.getDrawable(R.styleable.AbTabLayout_tabIndicatorResId);
        ...
        mIndicatorGravity = a.getInt(R.styleable.AbTabLayout_tabIndicatorGravity, GRAVITY_BOTTOM);
        mTabStrip.setCircleDotRadius(a.getDimensionPixelOffset(R.styleable.AbTabLayout_tabCircleDotRadius, 0));
        mTabStrip.setCircleDotColor(a.getColor(R.styleable.AbTabLayout_tabCircleDotColor, 0));
        mTabStrip.setIndicatorWidth(a.getDimensionPixelOffset(R.styleable.AbTabLayout_tabIndicatorWidth, 0));
        mTabStrip.setIndicatorHeight(
                a.getDimensionPixelOffset(R.styleable.AbTabLayout_tabIndicatorHeight, 0));
        mTabStrip.setIndicatorBackgroundColor(a.getColor(R.styleable.AbTabLayout_tabIndicatorBackgroundColor, 0));
        mTabStrip.setIndicatorBackgroundHeight(a.getDimensionPixelOffset(R.styleable.AbTabLayout_tabIndicatorBackgroundHeight, 0));

        mTabPaddingStart = mTabPaddingTop = mTabPaddingEnd = mTabPaddingBottom = a
                .getDimensionPixelOffset(R.styleable.AbTabLayout_tabPadding, 0);
        mTabPaddingStart = a.getDimensionPixelOffset(R.styleable.AbTabLayout_tabPaddingStart,
                mTabPaddingStart);
        mTabPaddingTop = a.getDimensionPixelOffset(R.styleable.AbTabLayout_tabPaddingTop,
                mTabPaddingTop);
        mTabPaddingEnd = a.getDimensionPixelOffset(R.styleable.AbTabLayout_tabPaddingEnd,
                mTabPaddingEnd);
        mTabPaddingBottom = a.getDimensionPixelOffset(R.styleable.AbTabLayout_tabPaddingBottom,
                mTabPaddingBottom);

        mTabTextSize = a.getDimensionPixelOffset(R.styleable.AbTabLayout_tabTexSize,
                getResources().getDimensionPixelOffset(R.dimen.tablayout_tab_text_size));
        mTabTextColors = a.getColorStateList(R.styleable.AbTabLayout_tabTextColor);

        mRequestedTabMinWidth = a.getDimensionPixelOffset(R.styleable.AbTabLayout_tabMinWidth,
                INVALID_WIDTH);
        mRequestedTabMaxWidth = a.getDimensionPixelOffset(R.styleable.AbTabLayout_tabMaxWidth,
                INVALID_WIDTH);
        mTabBackgroundResId = a.getResourceId(R.styleable.AbTabLayout_tabBackground, 0);
        mContentInsetStart = a.getDimensionPixelOffset(R.styleable.AbTabLayout_tabContentStart, 0);
        mContentInsetBottom = a.getDimensionPixelOffset(R.styleable.AbTabLayout_tabContentBottom, 0);
        mScrollMode = a.getInt(R.styleable.AbTabLayout_tabScrollMode, MODE_FIXED);
        mTabGravity = a.getInt(R.styleable.AbTabLayout_tabGravity, GRAVITY_CENTER);
        a.recycle();
    ...
    /**
     * Returns the current mode used by this {@link AbTabLayout}.
     *
     * @see #setTabScrollMode(int)
     */
    @Mode
    ...
    /**
     * The one-stop shop for setting up this {@link AbTabLayout} with a {@link ViewPager}.
     *
    ...
    private void setupWithViewPager(@Nullable final ViewPager viewPager, boolean autoRefresh,
        ...
        if (viewPager != null) {
            ...
            // Now we'll add a tab selected listener to set ViewPager's current rv_video_cell
            ...
            // Now update the scroll position to match the ViewPager's current rv_video_cell
    ...
    private void populateFromPagerAdapter() {
        ...
        if (mPagerAdapter != null) {
            ...
            // Make sure we reflect the currently set ViewPager rv_video_cell
...
//                animator.setStartDelay(100); 修复tab过多进行scroll时Indicator抖动问题
     ...
     * necessary calls back to the provided {@link AbTabLayout} so that the tab position is
     * kept in sync.
     *
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        private final WeakReference<AbTabLayout> mTabLayoutRef;
        private int mPreviousScrollState;
        private int mScrollState;

        public TabLayoutOnPageChangeListener(AbTabLayout tabLayout) {
        ...
        public void onPageScrolled(final int position, final float positionOffset,
            ...
            final AbTabLayout tabLayout = mTabLayoutRef.get();
            if (tabLayout != null) {
                // Only update the text selection if we're not settling, or we are settling after
                // being dragged
                final boolean updateText = mScrollState != RecyclerView.SCROLL_STATE_SETTLING ||
...
//                        && mPreviousScrollState == SCROLL_STATE_IDLE);
        ...
        public void onPageSelected(final int position) {
            ...
            if (tabLayout != null && tabLayout.getSelectedTabPosition() != position
                ...
                final boolean updateIndicator = mScrollState == AbsListView.OnScrollListener.SCROLL_STATE_IDLE
        ...
        private void reset() {
            mPreviousScrollState = mScrollState = AbsListView.OnScrollListener.SCROLL_STATE_IDLE;
        }
    }
