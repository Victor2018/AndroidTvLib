...
public class VideoActivity extends BaseActivity implements TvRecyclerView.OnItemListener,
    ...
    private TvRecyclerView mTrvVideo;
    ...
    Handler mHandler = new Handler(){
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case LOAD_MORE_DATA:
                    videoAdapter.appendDatas(getMovietDatas(20)); //加载数据
                    mTrvVideo.setLoadingMore(false); //加载数据完毕
                    mTrvVideo.setHasMoreData(true);
                    break;
            }
        }
    };
    ...
    private void initialize () {
        mTrvVideo = (TvRecyclerView) findViewById(R.id.trv_video);
        // 设置布局的横纵间距
        mTrvVideo.setSpacingWithMargins(30, 30);

        videoAdapter = new VideoAdapter(this);
        videoAdapter.setDatas(getMovietDatas(20));
        mTrvVideo.setAdapter(videoAdapter);
    ...
    private void setListener() {
        mTrvVideo.setOnItemListener(this);
        //边界监听
        mTrvVideo.setOnInBorderKeyEventListener(this);
        mTrvVideo.setOnLoadMoreListener(this);
    }
    ...
    public boolean onLoadMore() {
        ...
        mTrvVideo.setLoadingMore(true); //正在加载数据
